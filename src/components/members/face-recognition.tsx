...
export function FaceRecognition({ members, activeEvents = [], initialCheckedMemberIds = [] }: FaceRecognitionProps) {
  ...
  // 사용자가 명시적으로 토글한 항목만 별도 추적 → 자동 매치는 기본 선택 / 수동 매치는 기본 미선택
  ...
  const selectedMatchIds = useMemo(() => {
    ...
    autoMatches.forEach((m) => {
      if (!userDeselected.has(m.member.id)) ids.add(m.member.id)
    })
    manualMatches.forEach((m) => {
      if (userSelected.has(m.member.id)) ids.add(m.member.id)
    })
    return ids
  }, [autoMatches, manualMatches, userDeselected, userSelected])

  // ── 라이브 영상 좌표 변환 (object-cover 기준) ──
  const videoContainerRef = useRef<HTMLDivElement>(null)
  const [containerSize, setContainerSize] = useState<{ width: number; height: number } | null>(null)

  useLayoutEffect(() => {
    if (status !== 'viewfinder') return
    const measure = () => {
  ...
  const videoTransform = useMemo<VideoTransform | null>(() => {
    if (!videoSize || !containerSize) return null
    ...
    return {
      scale,
      offsetX: (containerSize.width - videoSize.width * scale) / 2,
      offsetY: (containerSize.height - videoSize.height * scale) / 2,
    }
  ...
  const toggleMatch = (memberId: string) => {
    if (selectedMatchIds.has(memberId)) {
      // 선택 해제
      setUserSelected((prev) => {
        if (!prev.has(memberId)) return prev
        const next = new Set(prev); next.delete(memberId); return next
      })
      setUserDeselected((prev) => new Set(prev).add(memberId))
    } else {
      // 선택
      setUserDeselected((prev) => {
        if (!prev.has(memberId)) return prev
        const next = new Set(prev); next.delete(memberId); return next
      })
      setUserSelected((prev) => new Set(prev).add(memberId))
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 bg-black/80 backdrop-blur-sm">
        ...
        <h1 className="text-white font-semibold">실시간 얼굴 인식</h1>
        <div className="flex items-center gap-3">
          ...
          {status === 'viewfinder' && allMatches.length > 0 && (
            <button
              onClick={clearMatches}
              className="text-xs text-white/80 font-medium px-2 py-1 border border-white/30 rounded-lg active:opacity-70"
            >
              초기화
            </button>
          )}
        ...
        <div className={cn(
          ...
          status !== 'viewfinder' && 'hidden'
            ...
            <div ref={videoContainerRef} className="relative bg-black flex-shrink-0" style={{ height: '50dvh' }}>
              ...
              {/* 실시간 얼굴 박스 */}
              {videoTransform && liveDetections.map((match) => {
                const isAuto = match.distance <= AUTO_CHECK_THRESHOLD
                const left = videoTransform.offsetX + match.box.x * videoTransform.scale
                const top = videoTransform.offsetY + match.box.y * videoTransform.scale
                const w = match.box.width * videoTransform.scale
                const h = match.box.height * videoTransform.scale
              ...
              <div className="absolute top-4 left-0 right-0 flex justify-center pointer-events-none">
                <div className="bg-black/60 backdrop-blur-sm text-white text-sm px-4 py-2 rounded-full">
                  {allMatches.length === 0
                    ? '단원이 보이도록 카메라를 맞춰주세요'
                    : `${allMatches.length}명 인식됨 · 자동 ${autoMatches.length} / 확인 ${manualMatches.length}`}
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 bg-white overflow-y-auto min-h-0 pb-safe">
              ...
              {allMatches.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12 gap-3 text-muted">
                  ...
                  <p className="text-sm">아직 인식된 단원이 없습니다</p>
                  <p className="text-xs">카메라를 단원 쪽으로 향해주세요</p>
                </div>
              )}
